// ═══════════════════════════════════════════════════════════
//  Clean Food Chiang Rai · script.js  v2.1
// ═══════════════════════════════════════════════════════════
...
/** คำนวณวันที่ส่งถัดไป และแปลงเป็นภาษาไทย */
...
/** Validate เบอร์โทรไทย: 0X-XXXX-XXXX หรือ 0XXXXXXXXX */
...
function getCategoryLabel(cat) {
    const labels = { chicken: "🍗 อกไก่", fish: "🐟 ปลา", beef: "🥩 เนื้อ", salad: "🥗 สลัด" };
...
function initMap() {
    ...
    const updateCoords = () => {
        ...
        const gpsEl = document.getElementById("gps-coords-text");
        const gpsDv = document.getElementById("gps-display");
        if (gpsEl) gpsEl.innerText = `${lat}, ${lng}`;
...
function editCartQty(index, change) {
    ...
    if (newQty <= 0) {
        // ลบโดยไม่ต้อง confirm — ใช้ toast แทน
        const removedName = cart[index].name;
        cart.splice(index, 1);
        updateCartUI();
        showToast(`ลบ "${removedName}" ออกจากตะกร้าแล้ว`);
    } else {
...
function checkFormValidity() {
    ...
    // Validate: ต้องกรอกครบ + เบอร์ถูกรูปแบบ + ปักหมุดแล้ว
    ...
    // แสดง error ใต้ช่อง tel ถ้ากรอกแล้วแต่ผิดรูปแบบ
    if (tel && !telOk) {
        setFieldError("cust-tel", "กรุณากรอกเบอร์โทรให้ถูกต้อง (10 หลัก)");
    } else {
        setFieldError("cust-tel", "");
    }
}
...
function generateQR() {
    ...
    if (!address)                 { showToast("⚠️ กรุณาปักหมุดในแผนที่ก่อน"); hasError = true; }
    if (!pdpa)                    { showToast("⚠️ กรุณายอมรับนโยบาย PDPA"); hasError = true; }
    if (hasError) return;
...
async function sendOrderToLINE() {
    ...
    try {
        ...
        // 3. เปิด LINE Chat ร้าน
        const lineUrl = `https://line.me/R/oaMessage/@282ovoyd/?${encodeURIComponent(lineMsg)}`;
        showToast("✅ บันทึกแล้ว! กำลังเปิดแชทร้าน...");
        setTimeout(() => { window.location.href = lineUrl; }, 800);
